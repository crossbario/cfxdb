    ...
    def oid(self) -> uuid.UUID:
        ...
        if self._oid is None and self._from_fbs:
            if self._from_fbs.MarketLength():
                _oid = self._from_fbs.MarketAsBytes()
...
@table('861b0942-0c3f-4d41-bc35-d8c86af0b2c9', build=Catalog.build, cast=Catalog.cast)
class Catalogs(MapUuidFlatBuffers):
    """
    Catalogs table, mapping from ``catalog_id|UUID`` to :class:`cfxdb.xbr.Catalog`
    """
