Release Notes
=============

This page lists all releases of **cfxdb**.

Current Release
---------------

The current stable release is available on
`PyPI <https://pypi.org/project/cfxdb/>`_.

.. code-block:: bash

    pip install cfxdb

Release History
---------------

For a complete list of releases, see:

* `PyPI Release History <https://pypi.org/project/cfxdb/#history>`_
* `GitHub Releases <https://github.com/crossbario/cfxdb/releases>`_

Release Notes
-------------

Detailed release notes and changes for each version are documented
in the :doc:`changelog`.

Version Policy
--------------

cfxdb follows `Semantic Versioning <https://semver.org/>`_:

* **Major** versions for incompatible API changes
* **Minor** versions for backwards-compatible functionality
* **Patch** versions for backwards-compatible bug fixes
